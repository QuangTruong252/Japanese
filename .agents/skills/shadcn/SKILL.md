...
For this repository, run `pnpm exec shadcn info --json` from `web/` before working on components. Use the installed CLI (`pnpm exec shadcn`) for the commands below, as required by the project rules. Do not run an automatic `npx ...@latest` command from the repository root.

The command returns the project config and installed components. Use `pnpm exec shadcn docs <component>` from `web/` to get documentation and example URLs for any component.
